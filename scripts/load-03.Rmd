...
sum_data %>% inner_join(sum_data %>% filter(rank<=10) %>% distinct(symbol),by='symbol') %>%
  ...
  group_walk(~{
    .y %>% kable() %>% print()
    .x %>% filter(rank<=10) %>% 
      summarise(cus_sprd=sum(cus_sprd), 
                fwd_sprd=sum(fwd_sprd), mkt_sprd=sum(mkt_sprd), filled=sum(filled), ratio=cus_sprd/mkt_sprd,
                value=sum(value)) %>%
      kable( digits = c(rep(0,4),4,0),
            format.args = list(big.mark=',')) %>% 
      print()
    .x %>% filter(rank<=10) %>% arrange(rank) %>% 
      select(-rank) %>%
      kable(digits=c(rep(0,4),rep(4,5),rep(0,1)),
            format.args = list(big.mark=',')) %>% 
      print()
    cat('\n\\newpage  \n')
  })
